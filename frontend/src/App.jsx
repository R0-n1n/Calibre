...
var WRAP_S = {
  minHeight: "100vh",
  background: "linear-gradient(160deg,#07070f 0%,#0d0d1c 60%,#0a0a16 100%)",
...
var LOGO_S = {
  ...
  borderRadius: 10,
  flexShrink: 0,
  background: "linear-gradient(135deg,#7c6fff,#4a3fbf)",
  display: "flex",
  alignItems: "center",
...
function bigBtnS(ok) {
  return {
    ...
    borderRadius: 12,
    background: ok
      ? "linear-gradient(135deg,#7c6fff,#4a3fbf)"
...
function tabBS(a) {
  return {
    ...
    fontWeight: 600,
    cursor: "pointer",
    background: a ? "rgba(124,111,255,0.2)" : "transparent",
    color: a ? "#b0a8ff" : "#666",
    outline: a ? "1px solid rgba(124,111,255,0.35)" : "none",
  };
}
function modeBS(a) {
  return {
    ...
    padding: "12px 20px",
    border:
      "1px solid " + (a ? "rgba(124,111,255,0.5)" : "rgba(255,255,255,0.08)"),
    borderRadius: 12,
    background: a ? "rgba(124,111,255,0.15)" : "rgba(255,255,255,0.03)",
    color: a ? "#b0a8ff" : "#666",
    cursor: "pointer",
    fontSize: 14,
    fontWeight: 600,
...
function dropS(ov) {
  return {
    border: "2px dashed " + (ov ? "#7c6fff" : "rgba(255,255,255,0.1)"),
    borderRadius: 12,
    padding: "36px 20px",
    textAlign: "center",
    cursor: "pointer",
...
function cpBtnS(d) {
  return {
    padding: "7px 14px",
    border: "1px solid " + (d ? "#4ade8040" : "rgba(255,255,255,0.1)"),
    borderRadius: 8,
    fontSize: 12,
    fontWeight: 600,
    cursor: "pointer",
    background: d ? "rgba(74,222,128,0.12)" : "rgba(255,255,255,0.05)",
    color: d ? "#4ade80" : "#888",
  };
}
function purpBS(l) {
...
function priTheme(p) {
  var m = {
    High: {
      ...
      c: "#f87171",
      bb: "rgba(248,113,113,0.18)",
    },
    Medium: {
      bg: "rgba(251,191,36,0.08)",
      bd: "rgba(251,191,36,0.22)",
      c: "#fbbf24",
      bb: "rgba(251,191,36,0.18)",
    },
    Low: {
      bg: "rgba(74,222,128,0.07)",
      bd: "rgba(74,222,128,0.2)",
      c: "#4ade80",
      bb: "rgba(74,222,128,0.15)",
    },
  };
  return (
    m[p] || {
  ...
  return (
    <div style={{ position: "relative", width: s, height: s, flexShrink: 0 }}>
      ...
      >
        ...
        <span style={{ fontSize: s * 0.085, color: "#555", marginTop: 2 }}>
          / 100
        </span>
      </div>
    </div>
...
function Card({ children, style, onClick }) {
  return (
    <div style={Object.assign({}, CARD_S, style || {})} onClick={onClick}>
      {children}
    </div>
  );
}
...
export default function App() {
  ...
  async function analyze() {
    ...
    try {
      ...
      setRT(data.resume_text || "");
      setResults(data);
      setTab("overview");
    } catch (e) {
      setError(
        e.message || "Analysis failed — make sure the backend is running.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function compareAll() {
    ...
    if (!resumeFile || !valid.length) {
      setError("Upload a resume and fill in at least one job.");
      return;
    }
    setCmpL(true);
    setError("");
    ...
    try {
      var all = await Promise.all(
        valid.map(async function (job) {
          var fd = new FormData();
          fd.append("resume", resumeFile);
          fd.append("job_description", job.jd);
          var d = await apiPost("/api/analyze", fd);
          if (!resumeText && d.resume_text) setRT(d.resume_text);
          return Object.assign({}, d, {
            company: job.company || "Job " + job.id,
            jd: job.jd,
          });
        }),
      );
  ...
  async function rewrite(sec) {
    ...
    if (currentCount >= 3) {
      setError("Rewrite limit reached for this section (3/3).");
      return;
    }
    ...
    try {
      ...
      fd.append("content", orig);
      fd.append("job_description", jobDesc);
      var data = await apiPost("/api/rewrite", fd);
      setRewrites(function (p) {
        return Object.assign({}, p, { [sec]: data.rewritten });
      });
      setRewriteCounts(function (p) {
        return Object.assign({}, p, { [sec]: (p[sec] || 0) + 1 });
      });
    } catch (e) {
      setError("Rewrite failed: " + (e.message || "unknown error"));
    } finally {
      setRewriting(null);
    }
  }
  ...
  async function genCover() {
    ...
    var resumeData =
      resumeText ||
      Object.entries(r.resume_sections || {})
        .filter(function (kv) {
          return kv[1];
        })
        .map(function (kv) {
    ...
    try {
      var fd = new FormData();
      fd.append("resume_text", resumeData);
      fd.append("job_description", jobDesc);
      fd.append("analysis", JSON.stringify(r));
      var data = await apiPost("/api/cover-letter", fd);
      setCover(data.cover_letter);
    } catch (e) {
      setError("Cover letter failed: " + (e.message || "unknown error"));
    } finally {
      setCoverL(false);
    }
  }
          ...
          <div>
            <div style={{ textAlign: "center", marginBottom: 32 }}>
              ...
              <p style={{ fontSize: 14, color: "#555" }}>
                ATS score · Skill gap · Interview prep · Gap projects · Cover
                letter
              </p>
            </div>
            <div style={{ display: "flex", gap: 10, marginBottom: 22 }}>
              <button
                style={modeBS(mode === "single")}
                onClick={function () {
                  setMode("single");
                }}
              >
            ...
            <Card style={{ marginBottom: 18 }}>
              ...
              <div
                ...
                onClick={function () {
                  document.getElementById("rf-input").click();
                }}
              >
                <input
                  id="rf-input"
                ...
                {resumeFile ? (
                  ...
                  <div>
                    ...
                    <div
                      style={{ color: "#bbb", fontWeight: 600, fontSize: 14 }}
                    >
                      Drop your resume here
                    </div>
                    <div style={{ color: "#444", fontSize: 12, marginTop: 5 }}>
                ...
                {jobs.map(function (job, idx) {
                  return (
                    <Card key={job.id} style={{ marginBottom: 12 }}>
                      ...
                      <input
                        style={Object.assign({}, INP_S, { marginBottom: 10 })}
                      ...
                      />
                      <textarea
                        style={Object.assign({}, TA_S, { minHeight: 100 })}
            ...
            {mode === "single" && (
              <button
                style={bigBtnS(!loading && !!resumeFile && !!jd.trim())}
                onClick={analyze}
            ...
            >
              <div>
                ...
                <p style={{ fontSize: 13, color: "#555", margin: "4px 0 0" }}>
                  Ranked by compatibility — click any row for full analysis
                </p>
              </div>
              <button style={GHOST_S} onClick={resetAll}>
                ← New Analysis
              </button>
            </div>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: 12, color: "#f87171" }}>
                          {"❌ " + (r.missing_skills || []).length + " missing"}
                        </span>
                      </div>
                      <p
                        style={{
                      ...
                      >
                        {r.summary ? r.summary.substring(0, 140) + "…" : ""}
                      </p>
                    </div>
          ...
          <div>
            ...
            >
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                >
                  {selJob !== null ? "← Comparison" : "← New Analysis"}
                </button>
                {selJob !== null && (
                  <span
                    style={{ fontSize: 13, color: "#b0a8ff", fontWeight: 600 }}
                  >
                    {ar.company}
                  </span>
                )}
            ...
            {tab === "overview" && (
              <div>
                  ...
                  <div style={{ flex: 1 }}>
                    <div
                      style={{
                        ...
                        borderRadius: 20,
                        marginBottom: 10,
                        background: verdictCol(ar.verdict) + "14",
                        border: "1px solid " + verdictCol(ar.verdict) + "35",
                      }}
                    >
                      <span>
                        {ar.verdict === "Strong Match"
                          ? "✅"
                          : ar.verdict === "Partial Match"
                            ? "⚠️"
                            : "❌"}
                      </span>
                      <span
                ...
                >
                  <Card>
                    <div style={LABEL_S}>✅ Matched Skills</div>
                    {(ar.matched_skills || []).length ? (
                      (ar.matched_skills || []).map(function (s, i) {
                        return <Chip key={i} label={s} color="#4ade80" />;
                      })
                    ) : (
                  ...
                  <Card>
                    <div style={LABEL_S}>❌ Missing Skills</div>
                    {(ar.missing_skills || []).length ? (
                      (ar.missing_skills || []).map(function (s, i) {
                        return <Chip key={i} label={s} color="#f87171" />;
                      })
                    ) : (
                      <span style={{ color: "#444", fontSize: 13 }}>
                        No major gaps
                ...
                {(ar.ats_warnings || []).length > 0 && (
                  <Card>
                    <div style={LABEL_S}>⚠️ ATS Risk Flags</div>
                    {(ar.ats_warnings || []).map(function (w, i) {
                      return (
                        <div
                          key={i}
                          style={{
            ...
            {tab === "interview" && (
              <div>
                <p style={{ fontSize: 13, color: "#555", marginBottom: 18 }}>
                  {(ar.interview_questions || []).length} predicted questions.
                  Tap each to see how to answer.
                </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          style={{ color: "#555", fontSize: 14, flexShrink: 0 }}
                        >
                          {open ? "▲" : "▼"}
                        </span>
                      </div>
            ...
            {tab === "gaps" && (
              <div>
                <p style={{ fontSize: 13, color: "#555", marginBottom: 18 }}>
                  For each missing skill — a concrete project to build it and
                  add to your resume.
                </p>
                {(ar.gap_projects || []).length > 0 ? (
                  (ar.gap_projects || []).map(function (g, i) {
                  ...
                  <Card>
                    <p style={{ color: "#555", margin: 0, fontSize: 13 }}>
                      No major skill gaps — strong position!
                    </p>
                  </Card>
                )}
              </div>
              ...
              <div>
                {!cover && !coverLoading && (
                  <Card style={{ textAlign: "center", padding: "44px 32px" }}>
                    ...
                    <h3
                      style={{ color: "#fff", marginBottom: 8, fontSize: 19 }}
                    >
                    ...
                    >
                      Personalized to this role. Highlights your strengths and
                      addresses gaps strategically.
                    </p>
                    <button
                      style={{
                        padding: "13px 36px",
                  ...
                  <div>
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <button
                          style={cpBtnS(copied === "cover")}
                          onClick={function () {
                            copyText(cover, "cover");
                          }}
                        >
            ...
            {tab === "tips" && (
              <div>
                <p style={{ fontSize: 13, color: "#555", marginBottom: 18 }}>
                  {(ar.improvement_tips || []).length +
                    " suggestions — tackle High priority first."}
                </p>
            ...
            {tab === "rewriter" && (
              <div>
                <p style={{ fontSize: 13, color: "#555", marginBottom: 18 }}>
                  Click <strong style={{ color: "#b0a8ff" }}>Rewrite</strong> on
                  any section for a JD-tailored version. Each section allows up
                  to 3 rewrites.
                </p>
